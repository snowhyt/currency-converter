...
export async function copyTracedFiles({ buildOutputPath, packagePath, outputDir, routes, bundledNextServer, skipServerFiles, }) {
    ...
    const extractFiles = (files, from = standaloneNextDir) => files.map((f) => path.resolve(from, f));
    ...
    const computeCopyFilesForPage = (pagePath) => {
        ...
        let requiredFiles;
        try {
            requiredFiles = JSON.parse(readFileSync(path.join(standaloneNextDir, `${fullFilePath}.nft.json`), "utf8"));
        }
        catch (e) {
...
--------------------------------------------------------------------------------`);
        ...
        filesToCopy.set(path.join(standaloneNextDir, fullFilePath), path.join(outputNextDir, fullFilePath));
    };
    ...
    readdirSync(path.join(standaloneNextDir, "server"))
        .filter((fileOrDir) => !statSync(path.join(standaloneNextDir, "server", fileOrDir)).isDirectory())
        .filter((file) => file !== "server.js")
        .forEach((file) => copyFileSync(path.join(standaloneNextDir, "server", file), path.join(path.join(outputNextDir, "server"), file)));
    // Copy patch file
    ...
    if (hasPageDir) {
        ...
        const staticFiles = Object.values(JSON.parse(readFileSync(path.join(standaloneNextDir, "server/pages-manifest.json"), "utf8")));
        // Then we need to get all fallback: true dynamic routes html
        const prerenderManifest = JSON.parse(readFileSync(path.join(standaloneNextDir, "prerender-manifest.json"), "utf8"));
        const config = JSON.parse(readFileSync(path.join(standaloneNextDir, "required-server-files.json"), "utf8")).config;
